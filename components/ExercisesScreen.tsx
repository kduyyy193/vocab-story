...
const ExercisesScreen: React.FC = () => {
    const { words, progress, updateWordProgress } = useVocabulary();
    ...
    const generateNewSession = () => {
        ...
        const newQuestions = sessionWords.map(questionWord => {
            const correctAnswer = questionWord.meaning;
            const wrongAnswers = shuffleArray(words.filter(w => w.id !== questionWord.id))
                .slice(0, 3)
                .map(w => w.meaning);
    ...
    const handleAnswer = (answer: string) => {
        ...
        if (correct) {
            ...
            updateWordProgress(questions[currentIndex].word.id, ReviewAction.Good);
        } else {
            updateWordProgress(questions[currentIndex].word.id, ReviewAction.Again);
        }
    };
